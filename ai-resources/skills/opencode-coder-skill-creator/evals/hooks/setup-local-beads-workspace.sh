...
hook_artifacts_dir="${artifacts_root}/hooks/setup-local-beads-workspace"
...
# Runner contract sets cwd to workspace root. Keep setup strictly local to cwd.
if [ ! -d .git ]; then
...
{
  printf 'hook=setup-local-beads-workspace\n'
...
bd list --status open >"${hook_artifacts_dir}/bd-open-issues.txt"
