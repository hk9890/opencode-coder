...
from scripts.utils import parse_skill_md, replace_frontmatter_description
...
def run_loop(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        t0 = time.time()
        iteration_artifacts_dir = (
            (log_dir.parent / "eval-artifacts" / f"iteration-{iteration}")
            if log_dir
            else skill_path / "eval-artifacts" / f"iteration-{iteration}"
        )
        all_results = run_eval(
            ...
            timeout=timeout,
            artifacts_dir=iteration_artifacts_dir,
...
def main():
    ...
    parser.add_argument(
        "--report",
        default="none",
        help="Generate HTML report at this path (default: 'none' to disable, 'auto' for temp file)",
    )
    parser.add_argument(
        "--results-dir",
        default=None,
    ...
    parser.add_argument(
        "--apply-best",
        action="store_true",
        help="Apply best_description into SKILL.md frontmatter description",
    )
    ...
    if args.apply_best:
        ...
        updated = replace_frontmatter_description(
            skill_md.read_text(),
            output["best_description"],
        )
        skill_md.write_text(updated)
        print(
            f"Applied best description to: {skill_md}",
            file=sys.stderr,
        )
