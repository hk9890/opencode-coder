...
describe.skipIf(!aimgrCheck.available)("coder-docs additive isolated loadability", () => {
  it("installs package/coder-docs into an isolated workspace without requiring root manifest edits", async () => {
    ...
    try {
      ...
      const installResult = await $`aimgr install package/coder-docs`.cwd(workspace.workdir).env(isolatedEnv).quiet();
      expect(installResult.exitCode).toBe(0);

      const skillPath = join(workspace.workdir, ".opencode", "skills", "coder-docs", "SKILL.md");
      const installedSkill = await readFile(skillPath, "utf8");
      expect(installedSkill).toContain("name: coder-docs");
      ...
      expect(rootManifest).not.toContain("package/coder-docs");
    } finally {
