...
describe.skipIf(!aimgrCheck.available)("coder-beads additive isolated loadability", () => {
  it("installs package/coder-beads into an isolated workspace without requiring root manifest edits", async () => {
    ...
    try {
      ...
      const installResult = await $`aimgr install package/coder-beads`.cwd(workspace.workdir).env(isolatedEnv).quiet();
      expect(installResult.exitCode).toBe(0);

      const skillPath = join(workspace.workdir, ".opencode", "skills", "coder-beads", "SKILL.md");
      const installedSkill = await readFile(skillPath, "utf8");
      expect(installedSkill).toContain("name: coder-beads");
      ...
      expect(rootManifest).not.toContain("package/coder-beads");
    } finally {
