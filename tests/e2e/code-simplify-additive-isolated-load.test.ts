...
describe.skipIf(!aimgrCheck.available)("code-simplify additive isolated loadability", () => {
  it("installs package/code-simplify into an isolated workspace without requiring root manifest edits", async () => {
    ...
    try {
      ...
      const installResult = await $`aimgr install package/code-simplify`.cwd(workspace.workdir).env(isolatedEnv).quiet();
      expect(installResult.exitCode).toBe(0);

      const skillPath = join(workspace.workdir, ".opencode", "skills", "code-simplify", "SKILL.md");
      const installedSkill = await readFile(skillPath, "utf8");
      expect(installedSkill).toContain("name: code-simplify");
      ...
      expect(rootManifest).not.toContain("package/code-simplify");
    } finally {
